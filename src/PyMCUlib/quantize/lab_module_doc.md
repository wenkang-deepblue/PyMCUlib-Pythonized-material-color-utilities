...
Standard D65 white point used in color space conversions, imported from `PyMCUlib.utils.utils`.

## Dependencies

The module depends on the following imports:
- `NamedTuple` from `typing`
- `Argb`, `WHITE_POINT_D65`, `delinearized`, `linearized`, `argb_from_rgb` from `PyMCUlib.utils.utils`
...
- The `Argb` type is a type alias for an integer, defined in the `PyMCUlib.utils.utils` module.