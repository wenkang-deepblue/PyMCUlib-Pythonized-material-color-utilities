...
```python
from PyMCUlib_cpp.palettes import TonalPalette
...
```python
from PyMCUlib_cpp.palettes import TonalPalette, CorePalettes
from PyMCUlib_cpp.utils.utils import hex_from_argb
...
This component depends on the following PyMCUlib_cpp modules:
* `PyMCUlib_cpp.utils.utils` - For ARGB color manipulation utilities
* `PyMCUlib_cpp.cam.cam` - For color appearance model functions
* `PyMCUlib_cpp.cam.hct` - For the HCT color space representation