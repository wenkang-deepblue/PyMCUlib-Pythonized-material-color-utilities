...
## Dependencies
* `PyMCUlib_cpp.utils.utils`: For basic color manipulation functions
...
* `PyMCUlib_cpp.quantize.lab`: For Lab color space conversions
...
```python
from PyMCUlib_cpp.quantize.wsmeans import quantize_wsmeans
from PyMCUlib_cpp.utils.utils import hex_from_argb, argb_from_rgb
...
The WSMeans quantizer can be used with other quantization methods like Wu's algorithm. The `quantize_celebi` function in `PyMCUlib_cpp.quantize.celebi` uses Wu's quantizer to generate initial clusters, then refines them using WSMeans.
