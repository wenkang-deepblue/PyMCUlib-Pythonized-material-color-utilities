# <center> Content Module Documentation </center>

## Overview
`SchemeContent` is a class that generates a content color scheme based on a source color. This color scheme is designed to be closer to the original source color, with less dramatic color transformations compared to other schemes. The content scheme is ideal for applications where staying true to the user's selected color is a priority.

The content scheme preserves the original hue and chroma for primary colors, reduces chroma slightly for secondary colors, and uses an analogous hue for tertiary colors, creating a harmonious palette that maintains the essence of the source color.
...
high_contrast_scheme = SchemeContent(source_color, is_dark, contrast_level=0.5)
```
...
The `SchemeContent` class generates its color palettes using the following processes:

1. __Primary Palette__: Uses the source color's original hue and chroma without modification.
...
3. __Tertiary Palette__: Uses an analogous color obtained from the `TemperatureCache`. The tertiary color is the third color in a set of analogous colors and is processed through the `fix_if_disliked` function to ensure harmonious color results.
...
* `TemperatureCache`: Provides a system for generating analogous colors based on color temperature principles.
...
## Available Colors
Through inheritance from `DynamicScheme`, `SchemeContent` provides access to a wide range of Material Design 3 colors, including:
...
primary = scheme.get_primary()  # Main brand color (close to original)
primary_container = scheme.get_primary_container()  # Container color derived from primary
secondary = scheme.get_secondary()  # Secondary color (slightly reduced chroma)
tertiary = scheme.get_tertiary()  # Accent color (analogous hue)
